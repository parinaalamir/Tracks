function User() {
    ...
    if(UCounter==1){
        Messages[Messages.map((x, i) => [i, x]).filter(
            x => x[1] == "Hi, what's the name of your company?")[0][0]] = "Hi " + ans + ", what's the name of your company?"
    }
    if(UCounter==5) {
        if (/and/.test(ans)) {
            ...
            Messages[Messages.map((x, i) => [i, x]).filter(
                x => x[1] == "How many Volvo trucks do you have?")[0][0]] = "How many " + ans1 + " trucks do you have?"
        } else if (/,/.test(ans)) {
            var str = ans.toString();
            ...
            Messages[Messages.map((x, i) => [i, x]).filter(
                x => x[1] == "How many Volvo trucks do you have?")[0][0]] = "How many " + ans + " trucks do you have?"
        }
    }
}